...
namespace Mesen.Debugger.Utilities
{
	public class PaletteHelper
	{
		public static DynamicTooltip GetPreviewPanel(UInt32[] rgbPalette, UInt32[] rawPalette, RawPaletteFormat format, int index, DynamicTooltip? tooltipToUpdate, int colorsPerPalette = 0)
		{
			...
			entries.AddEntry(ResourceHelper.GetMessage("PaletteHelper_Color"), new TooltipColorEntry(rgbPalette[index]));
			if(colorsPerPalette > 0) {
				entries.AddEntry(ResourceHelper.GetMessage("PaletteHelper_Index"), "$" + (index % colorsPerPalette).ToString("X2"));
			} else {
				entries.AddEntry(ResourceHelper.GetMessage("PaletteHelper_Index"), "$" + index.ToString("X2"));
			}

			if(format == RawPaletteFormat.Rgb555) {
				//SNES / GBC
				entries.AddEntry(ResourceHelper.GetMessage("PaletteHelper_Value"), "$" + rawPalette[index].ToString("X4"));
				...
				//PC Engine
				entries.AddEntry(ResourceHelper.GetMessage("PaletteHelper_Value"), "$" + rawPalette[index].ToString("X3"));
				...
				//SMS
				entries.AddEntry(ResourceHelper.GetMessage("PaletteHelper_Value"), "$" + rawPalette[index].ToString("X2"));
			...
			entries.AddEntry(ResourceHelper.GetMessage("PaletteHelper_ColorCodeHex"), "#" + rgbPalette[index].ToString("X8").Substring(2));
			entries.AddEntry(ResourceHelper.GetMessage("PaletteHelper_ColorCodeRGB"),
